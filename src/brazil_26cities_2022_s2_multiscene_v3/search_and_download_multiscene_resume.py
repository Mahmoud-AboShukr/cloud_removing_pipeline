#!/usr/bin/env python3
"""
search_and_download_multiscene_resume.py

Resume / backfill downloader for the 26-city Sentinel-2 V3 pipeline.

What this version does
----------------------
- Uses the same logic as the production downloader
- Skips cities that are already fully downloaded
- Re-downloads only cities that are missing / incomplete
- Reuses existing selected_scenes_summary.json when possible
- Preserves already-downloaded scene assets
- Supports running on only one city or all cities

Expected output structure
-------------------------
data/raw/<city_slug>/2022/s2_planetary_multiscene_12band_v3/
    <city_slug>_aoi.gpkg
...
USER_AGENT = "brazil-26cities-s2-v3-resume/1.0"
...
def count_existing_tifs(scene_dir: Path) -> int:
    if not scene_dir.exists():
        return 0
    return len(list(scene_dir.glob("*.tif")))


def scene_is_complete(scene_dir: Path, required_assets: Sequence[str]) -> bool:
    if not scene_dir.exists():
        return False
    for asset in required_assets:
        if not (scene_dir / f"{asset}.tif").exists():
            return False
    return True
...
# -----------------------------------------------------------------------------
# Existing-download inspection
# -----------------------------------------------------------------------------

def inspect_existing_city_download(
    raw_city_root: Path,
    required_assets: Sequence[str],
) -> Dict[str, Any]:
    summary_path = raw_city_root / "selected_scenes_summary.json"
    scenes_root = raw_city_root / "scenes"

    result = {
        "summary_exists": summary_path.exists(),
        "scenes_root_exists": scenes_root.exists(),
        "complete_scenes": [],
        "incomplete_scenes": [],
        "num_complete": 0,
        "num_incomplete": 0,
        "num_selected_in_summary": 0,
    ...
    try:
        ...
        logging.warning("Could not read existing summary %s: %s", summary_path, exc)
        return result

    selected = summary.get("selected_candidates", [])
    result["num_selected_in_summary"] = len(selected)
    ...
    for cand in selected:
        ...
        scene_dir = scenes_root / item_id
        complete = scene_is_complete(scene_dir, required_assets)
        tif_count = count_existing_tifs(scene_dir)
        ...
        if complete:
            result["complete_scenes"].append(scene_info)
        else:
            result["incomplete_scenes"].append(scene_info)

    result["num_complete"] = len(result["complete_scenes"])
    result["num_incomplete"] = len(result["incomplete_scenes"])
    result["is_fully_complete"] = (
        result["num_selected_in_summary"] > 0
        and result["num_complete"] == result["num_selected_in_summary"]
    )
...
def process_city(
    ...
    existing_state = inspect_existing_city_download(raw_city_root, assets_to_download)

    if existing_state["is_fully_complete"] and not force_redownload:
        logging.info(
            "City %s already fully downloaded (%d complete selected scenes). Skipping.",
            city_display,
            existing_state["num_complete"],
        )
        return {
            "city_key": city_key,
    ...
    if existing_state["summary_exists"] and not force_redownload:
        logging.info(
            "City %s has partial existing download: complete=%d incomplete=%d selected=%d",
            city_display,
            existing_state["num_complete"],
            existing_state["num_incomplete"],
            existing_state["num_selected_in_summary"],
        )
    ...
    while idx < len(effective_targets):
        ...
        # Skip if scene already complete
        if scene_is_complete(scene_dir, assets_to_download) and not force_redownload:
            logging.info(
                "Scene already complete, skipping download: %s",
                cand.item_id,
            )
            downloaded.append(
                DownloadedScene(
                    item_id=cand.item_id,
                    datetime_utc=cand.datetime_utc,
                    date=cand.date,
                    ...
                    scene_dir=str(scene_dir),
                    downloaded_assets={
                        asset: str(scene_dir / f"{asset}.tif")
                        for asset in assets_to_download
                        if (scene_dir / f"{asset}.tif").exists()
                    },
                )
            )
            idx += 1
        ...
        if result is not None and scene_is_complete(scene_dir, assets_to_download):
        ...
        logging.warning(
            "Scene %s failed to download completely. Attempting replacement candidate.",
            cand.item_id,
        )
    ...
    final_existing_state = inspect_existing_city_download(raw_city_root, assets_to_download)
    ...
    summary = {
        ...
        "existing_state_before": existing_state,
        "existing_state_after": final_existing_state,
    }
    ...
    lines.append("")
    lines.append(f"Downloaded / available scenes: {len(downloaded)}")
    lines.append(
        f"Complete selected scenes after run: {final_existing_state['num_complete']} / {final_existing_state['num_selected_in_summary']}"
    )
    write_text(raw_city_root / "selected_scenes_summary.txt", "\n".join(lines))

    logging.info(
        "Finished city %s. Complete selected scenes after run: %d/%d",
        city_display,
        final_existing_state["num_complete"],
        final_existing_state["num_selected_in_summary"],
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Resume / backfill multi-scene Sentinel-2 downloads for 26 Brazilian cities."
    )
    ...
    parser.add_argument(
        "--force-redownload",
        action="store_true",
        help="Force re-download even if a city already looks complete.",
    )
    parser.add_argument(
        "--only-missing",
        action="store_true",
        help="When using --city all, only process cities not yet complete.",
    )
    parser.add_argument(
        "--log-level",
        type=str,
...
def main() -> None:
    ...
    if args.only_missing and args.city == "all":
        filtered_items = []
        for city_key, city_cfg in city_items:
            city_slug = city_cfg["slug"]
            raw_city_root = args.output_root / city_slug / "2022" / "s2_planetary_multiscene_12band_v3"
            state = inspect_existing_city_download(raw_city_root, args.bands)
            if not state["is_fully_complete"]:
                filtered_items.append((city_key, city_cfg))
        city_items = filtered_items
        logging.info("Cities remaining after --only-missing filter: %d", len(city_items))
    ...
    overall_path = args.output_root / "brazil_26cities_2022_s2_multiscene_v3_resume_download_run_summary.json"
    json_dump(overall_results, overall_path)

    logging.info("Resume run finished.")
