#!/usr/bin/env python3
"""
render_composite_preview_v2.py

Create quick-look RGB previews from the v2 cloud-reduced 12-band Sentinel-2 composite.
...
def process_city(
    ...
    """
    Render preview for one city using v2 composite outputs.
    """
    city_slug = city_cfg["slug"]
    city_display = city_cfg["display_name"]

    logging.info("=" * 80)
    logging.info("Rendering v2 preview for city: %s", city_display)
    logging.info("=" * 80)

    city_dir = processed_root / city_slug / "2022" / "s2_planetary_multiscene_12band_v2"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Render RGB quick-look previews from v2 cloud-reduced composites."
    )
...
def main() -> None:
    ...
    logging.info("V2 preview rendering finished for %d city/cities.", len(results))
