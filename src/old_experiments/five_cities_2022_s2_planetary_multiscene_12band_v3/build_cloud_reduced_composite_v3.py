...
using dynamic filler ranking.

Key V3 changes relative to V2
-----------------------------
...
3. Dynamic filler ranking:
   - The base scene is chosen globally using usable ratio / overlap / cloud cover.
   - After that, remaining scenes are re-ranked at each iteration by how many of the
     currently missing pixels they can fill.
   - This is designed to improve cities where a globally weaker scene is still the
     best local filler for cloudy gaps.
...
def build_composite_for_city(
    ...
    # -------------------------------------------------------------------------
    # Step 1: Choose base scene globally
    # -------------------------------------------------------------------------
    ...
    while True:
        ...
        # Early stop only after minimum contributing scenes
        if (
        ...
        # ---------------------------------------------------------------------
        # Step 2+: Dynamically select next filler
        # ---------------------------------------------------------------------
        ...
        # If the best remaining scene cannot fill anything, stop
        if next_score.fillable_pixels <= 0:
            logging.info(
                "Best remaining scene cannot fill any additional pixels. Stopping."
            )
            break
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build stricter cloud-reduced multi-scene Sentinel-2 composites (v3 with dynamic filler ranking)."
    )
