...
Create quick-look RGB previews from the v3 cloud-reduced 12-band Sentinel-2 composite.
...
def process_city(
    ...
    logging.info("=" * 80)
    logging.info("Rendering v3 preview for city: %s", city_display)
    logging.info("=" * 80)

    city_dir = processed_root / city_slug / "2022" / "s2_planetary_multiscene_12band_v3"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render RGB quick-look previews from v3 cloud-reduced composites."
    )
...
def main() -> None:
    ...
    logging.info("V3 preview rendering finished for %d city/cities.", len(results))
