...
Batch search + download pipeline for 5 Brazilian cities using Sentinel-2 L2A
from Microsoft Planetary Computer.

What this script does
---------------------
1. Reads cleaned favela polygons from:
       data/interim/polygons_clean/favelas_clean.gpkg
2. Builds AOI as envelope(union(city polygons))
3. Searches Microsoft Planetary Computer STAC for Sentinel-2 L2A scenes in 2022
4. Applies:
       - seasonal-first search
       - overlap filtering
       - cloud filtering
       - deduplication
5. Selects multiple scenes per city
6. Downloads:
       B01, B02, B03, B04, B05, B06, B07, B08, B8A, B09, B11, B12, SCL
7. Saves summaries and diagnostics

Example
-------
python3 src/five_cities_2022_s2_planetary_multiscene_12band/search_and_download_multiscene.py
python3 src/five_cities_2022_s2_planetary_multiscene_12band/search_and_download_multiscene.py --city brasilia
python3 src/five_cities_2022_s2_planetary_multiscene_12band/search_and_download_multiscene.py --city all --max-scenes 6

Required companion file
-----------------------
This script expects:
    src/five_cities_2022_s2_planetary_multiscene_12band/cities_config.py
with a dictionary called CITY_CONFIGS.
"""
...
USER_AGENT = "favela-cloud-reduction-pipeline/1.0"
...
class SceneCandidate:
    """Metadata for a candidate Sentinel-2 scene."""
    ...
    def selection_score(self) -> float:
        """Higher is better."""
        overlap_score = self.overlap_ratio
        cloud_score = 1.0 - min(max(self.cloud_cover, 0.0), 100.0) / 100.0
        return 0.70 * overlap_score + 0.30 * cloud_score


@dataclass
class DownloadedScene:
    """Tracks one downloaded scene and local file paths."""
...
def load_city_polygons(
    ...
    """
    Load cleaned favela polygons and filter by city.
    """
    ...
    if city_gdf.empty:
        raise ValueError(
            f"No polygons found for city '{city_name_in_gpkg}' "
            f"in column '{city_column}'."
        )
...
def build_city_aoi(city_gdf: gpd.GeoDataFrame) -> Tuple[gpd.GeoDataFrame, BaseGeometry]:
    """
    Build AOI as envelope(union(city polygons)).
    """
...
def open_stac_client() -> Client:
    """Open Planetary Computer STAC client."""
    return Client.open(
        STAC_URL,
...
def compute_overlap_ratio(aoi_geom: BaseGeometry, item: Any) -> float:
    """
    Overlap ratio = intersection(item_bbox, aoi) / area(aoi)
    """
...
def signed_item(item: Any) -> Any:
    """Sign item for Planetary Computer."""
...
def scene_to_candidate(aoi_geom: BaseGeometry, item: Any) -> SceneCandidate:
    """Convert STAC item to SceneCandidate."""
...
def deduplicate_candidates(candidates: Sequence[SceneCandidate]) -> List[SceneCandidate]:
    """
    Deduplicate same datetime + tile.
    Keep best by overlap, then cloud.
    """
...
def sort_candidates_for_selection(candidates: Sequence[SceneCandidate]) -> List[SceneCandidate]:
    """
    Sort candidates with emphasis on overlap and low cloud.
    """
    return sorted(
        candidates,
        key=lambda c: (
...
def filter_candidates_by_overlap(
    ...
    """
    Keep preferred overlap if possible, else fallback overlap.
    """
    ...
    logging.warning(
        "No scenes reached preferred or fallback overlap thresholds. "
        "Keeping all candidates as last resort."
    )
...
def collect_candidates(
    ...
    """
    Seasonal-first search, then full-year fallback.
    """
...
def select_multiscene_set(
    ...
    """
    Select a good multi-scene set.

    Strategy:
    - pick best base scene
    - prefer diversity in dates
    - fill remaining slots with next-best scenes
    """
    if not candidates:
        return []
...
def fetch_item_by_id(client: Client, item_id: str) -> Any:
    """Fetch STAC item by ID from collection."""
...
def download_scene_assets(
    ...
    """
    Download required assets for one selected scene.
    Returns DownloadedScene on success, None on failure.
    """
    ...
    for asset_idx, asset_key in enumerate(assets_to_download, start=1):
        ...
        if not ok:
            logging.error(
                "Failed asset download for %s / %s", candidate.item_id, asset_key
            )
            return None
...
def process_city(
    ...
    raw_city_root = output_root / city_slug / "2022" / "s2_planetary_multiscene_12band"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Search and download multi-scene Sentinel-2 data for five cities in Brazil."
    )
...
def main() -> None:
    ...
    overall_path = (
        args.output_root
        / "five_cities_2022_s2_planetary_multiscene_12band_run_summary.json"
    )
